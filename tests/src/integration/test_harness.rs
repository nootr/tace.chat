...
use crate::integration::{NetworkSimulator, TimingController, SimulatorMessage, RequestId};
...
impl TestHarness {
    ...
    pub async fn add_node(&mut self, node_id: NodeId, p2p_port: u16, api_port: u16) -> Result<String, Box<dyn std::error::Error>> {
        ...
        let chord_node = Arc::new(ChordNode::new_with_client(
            node_id,
            &p2p_address,
            &api_address,
            network_client,
        ...
        // Store the node
        self.nodes.write().await.insert(p2p_address.clone(), test_node);
            ...
            if node.task_handle.is_none() {
                ...
                let handle = tokio::spawn(async move {
                    while let Some(sim_message) = receiver.recv().await {
                        ...
                        match sim_message {
                            SimulatorMessage::Request { from, message, request_id: _, response_sender } => {
                                let response = Self::process_dht_message(&chord_node, &from, message).await;
    ...
    pub async fn connect_node_to_network(&self, node_address: &str, bootstrap_address: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Some(bootstrap) = bootstrap_address {
            ...
            node.chord_node.join_network(Some(bootstrap)).await.map_err(|e| e.to_string())?;
        } else {
            // Create new network
            node.chord_node.create_network().await.map_err(|e| e.to_string())?;
        }
    ...
    pub async fn wait_for_stabilization(&self, max_wait_seconds: u64) -> Result<(), Box<dyn std::error::Error>> {
        ...
        loop {
            ...
            let mut all_have_successors = true;
            let mut ring_formed = true;
            
            for address in &node_addresses {
                if let Some(node) = self.get_node(address).await {
                    if node.get_successor().await.is_none() {
                        all_have_successors = false;
                        break;
                    }
                } else {
                    all_have_successors = false;
    ...
    pub async fn store_data(&self, node_address: &str, key: NodeId, value: Vec<u8>) -> Result<(), Box<dyn std::error::Error>> {
        let nodes = self.nodes.read().await;
        let node = nodes.get(node_address)
            .ok_or_else(|| format!("Node {} not found", node_address))?;

        node.chord_node.store_data(key, value).await.map_err(|e| e.to_string())?;
        Ok(())
    }
        ...
        match message {
            ...
            DhtMessage::GetSuccessor => {
                if let Some((id, address, api_address)) = chord_node.get_successor().await {
                    ...
                    Some(DhtMessage::Error { message: "No successor found".to_string() })
                }
            }
            ...
            DhtMessage::Notify { id, address, api_address } => {
            ...
            DhtMessage::Store { key, value } => {
                match chord_node.store_data(key, value).await {
                    ...
                    Err(e) => Some(DhtMessage::Error { message: e.to_string() })
                }
            }
            
            DhtMessage::Retrieve { key } => {
                match chord_node.retrieve_data(key).await {
            ...
            DhtMessage::ClosestPrecedingNode { id: _ } => {
                ...
                Some(DhtMessage::Error { message: "ClosestPrecedingNode not implemented in public API".to_string() })
            }
            ...
            DhtMessage::ShareMetrics { metrics: _ } => {
                ...
                Some(DhtMessage::MetricsShared { metrics: node_metrics })
            }
            
            DhtMessage::ReplicateData { key, values, replica_id: _ } => {
                // Store the replicated data
                for value in values {
                    if let Err(_) = chord_node.store_data(key, value).await {
                        return Some(DhtMessage::ReplicationAck { key, success: false });
                    }
                }
            ...
            DhtMessage::Error { .. } => {
                ...
                Some(DhtMessage::Error { message: "Received response message as request".to_string() })
            }
        }
    }
    ...
    pub async fn trigger_stabilization_cycles(&self, rounds: u32) -> Result<(), Box<dyn std::error::Error>> {
        for _ in 0..rounds {
